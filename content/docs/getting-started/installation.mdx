...
<Tabs items={["Linux", "macOS", "Windows"]}>
  ...
  <Tab>
    Download `scalp_windows_amd64.zip` from the [releases
    page](https://github.com/scal-p-labs/SCAL-P/releases), extract, and add to your PATH.
  </Tab>
</Tabs>
...
<Callout type="info">
  The binary is named `scalp` (no hyphen) regardless of installation method. The Go module path is
  `scal-p` (with hyphen), but the CLI command is always `scalp`.
</Callout>
...
<Callout type="info">
  This is dogfooding — SCAL-P verifies its own releases using the exact same engine it uses to
  verify your npm packages.
</Callout>
