...
<Callout type="info">
  **Prerequisite:** SCAL-P must be installed on your machine. See
  [Installation](/docs/getting-started/installation) if you haven't done this yet.
</Callout>
...
<Callout type="info">
  No config file needed. If you don't have a policy yet, SCAL-P uses a safe default: audit-only mode
  with warn enforcement.
</Callout>
...
<Callout type="warning">
  `--guarded` evaluates *before* install. If a violation is found and enforcement is `block`, the
  install never runs. No packages land in `node_modules`.
</Callout>
...
<Callout type="info">
  Commit `.scalp/policy.json` and `.scalp/policy.schema.json` to version control. Never commit
  `lockfile.json`, `cache/`, or `audit.log`.
</Callout>
