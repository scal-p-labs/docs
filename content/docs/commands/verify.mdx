...
```
INFO  binary verified: scalp_linux_amd64.tar.gz (sha512-a1b2...)
```

On mismatch:

```
WARN  binary mismatch: scalp_linux_amd64.tar.gz
...
```
ERROR binary mismatch: blocking
```
...
```json
{"ts":"2026-05-22T12:00:00Z","event":"binary_verify","pkg":"scalp_linux_amd64.tar.gz","status":"verified","hash_match":true}
```
