...
<Callout type="info">
  `npm stage download` is part of npm's private packages / staging feature. If you don't use npm
...
<Steps>
  ...
  <Step>**Enforcement** — `--ci` forces block. Otherwise uses policy's `on_violation`.</Step>
</Steps>
...
<Callout type="info">
  This streaming approach means SCAL-P can verify tarballs of any size without memory or disk
  overhead.
</Callout>
